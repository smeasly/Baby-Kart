...
func play(stream: AudioStreamSample, randomize_pitch: bool = false, volume: float = 0.0, min_pitch: float = 1.0, max_pitch: float = 1.2) -> void: #play(streamPath : String,...)
	...
	audioStreamPlayer.stream = stream #load(streamPath)
